...
def evaluate(p_emb, CF_data, LP_data, dataset, method, metric=['CF', 'LP', 'CL'], save_result=False, result_path='./result',
             random_state=123, max_iter=150,
             n_jobs=1, n_time=10, sim_matrix=True):
    evaluator = Evaluator(method, CF_data, LP_data, dataset, result_path, random_state, max_iter, n_jobs, sim_matrix)
    if 'CF' in metric:
        evaluator.evluate_CF(p_emb)
    if 'LP' in metric:
        evaluator.evluate_LP(p_emb)
    if 'CL' in metric:
        evaluator.evluate_CL(p_emb, time=n_time)
    if save_result:
